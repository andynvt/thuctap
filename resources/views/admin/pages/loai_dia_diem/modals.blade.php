    ...
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            <div class="modal-header">
                <h3 class="modal-title text-black-50 title" id="list-type" style="font-size: 16px">THÊM MỚI LOẠI DU
                    LỊCH</h3>
...
@foreach($placeType as $tp)
    <div class="modal fade" id="infoplace-{{$tp->id}}" tabindex="-1" role="dialog"
         aria-labelledby="list-info-placetype-{{$tp->id}}" aria-hidden="true">
        <div class="modal-dialog" role="document" style="max-width: 80%; min-height: 30%;">
            <div class="modal-content">
                <div class="modal-header text-center">
                    <h5 class="modal-title text-info title" id="list-info-placetype-{{$tp->id}}">DANH SÁCH LIÊN
                        QUAN</h5>
                    <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                        <span aria-hidden="true">&times;</span>
                    </button>
                ...
                <div class="modal-body">
                    <div class="table">
                        <table class="table" id="info-table">
                            <thead class="bg-primary" style="font-size: 12px; color: #ffffff;">
                            <th class="text-center">STT</th>
                            <th style="width: 20%">ĐỊA ĐIỂM DU LỊCH</th>
...
@foreach($placeType as $tp)
    <div class="modal fade" id="edit-placetype{{$tp->id}}" tabindex="-1" role="dialog"
         aria-labelledby="list-edit-placetype-{{$tp->id}}" aria-hidden="true">
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                <div class="modal-header text-center">
                    <h3 class="modal-title title text-black-50" id="list-edit-placetype-{{$tp->id}}"
                        style="font-size: 16px">CẬP NHẬT LOẠI DU LỊCH</h3>
                    <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                        <span aria-hidden="true">&times;</span>
                    </button>
                </div>
                <div class="modal-body">
                    <form action="{{route('admin.sua-loai',[$tp->id])}}" method="post">
                        ...
                        <div class="form-group">
                            <label class="bmd-label-floating">Tên loại du lịch</label>
                            <input type="text" class="form-control" name="name_type" id="name_type"
                                   value="{{$tp->name}}" minlength="5" required>
                        </div>
                        <div class="form-group">
                            <label class="bmd-label-floating">Mô tả</label>
...
{{--Modal delete--}}
@foreach($placeType as $tp)
    <div class="modal fade" id="modal-delete-{{$tp->id}}" tabindex="-1" role="dialog" aria-labelledby="delete" aria-hidden="true">
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                <div class="modal-header">
                ...
                <div class="modal-footer">
                    <form action="{{route('admin.xoa-loai',[$tp->id])}}">
                        {{ method_field('DELETE') }}
                        {{ csrf_field() }}
                        <button type="button" class="btn btn-secondary btn-round" data-dismiss="modal">ĐÓNG</button>
